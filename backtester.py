#!/usr/bin/env python3
"""
IOFAE Trading Bot - Advanced Backtester
Tests the IOFAE strategy with institutional execution probability.

Hedef: Kurumsal emirlerin ÖNÜNDE pozisyon almak.
"""
...
class IOFAEBacktester:
    """
    Advanced backtester implementing full IOFAE logic:
    
    1. Execution Zone Detection (VWAP, Round, Fib, DOM, Delta)
    2. Score ≥ 90 için trade
    3. 7 pip önce giriş
    4. Exhaustion detection ile çıkış
    5. Stop Hunt pattern tespiti
    """
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        exhaustion = self.config.get('exhaustion', {})
        self.volume_drop = exhaustion.get('volume_drop_threshold', 0.70)
        self.spread_widen = exhaustion.get('spread_widen_threshold', 1.50)
    ...
    def connect(self) -> bool:
        ...
        self.pip_value = info.point * 10 if info.digits == 5 else info.point
        
        self._connected = True
        print(f"✅ MT5 Connected | Symbol: {self.symbol}")
        return True
    ...
    def run_backtest(
        ...
        print(f"\n{'='*70}")
        print(f"🚀 IOFAE BACKTEST: {start_date.date()} → {end_date.date()}")
        print(f"{'='*70}")
        
        # Load data
        print("\n📊 Loading historical data...")
        bars_m1 = self._load_data(start_date, end_date, mt5.TIMEFRAME_M1)
        bars_d1 = self._load_data(start_date - timedelta(days=10), end_date, mt5.TIMEFRAME_D1)
        
        if bars_m1 is None or len(bars_m1) == 0:
            raise Exception("No M1 data available")
        ...
        # Volume/spread buffers for exhaustion
        ...
        for i, bar in enumerate(bars_m1.itertuples()):
            bar_time = bar.time
            ...
            try:
                spread = bars_m1.iloc[i]['high'] - bars_m1.iloc[i]['low']
            except:
                spread = 0.0001
            ...
            # Check session (London-NY overlap best)
            ...
            swing_high, swing_low = swing_cache.get(bar_date, (0, 0))
            fib_levels = self._calc_fib(swing_high, swing_low)
            
            # Check for stop hunt (08:00-08:30)
    ...
    def _load_data(self, start: datetime, end: datetime, tf) -> Optional[pd.DataFrame]:
    ...
    def _is_optimal_session(self, t: datetime) -> bool:
        """London-NY overlap: 07:00-17:00 UTC optimal."""
        h = t.hour
        return 7 <= h <= 17
    
    def _check_stop_hunt(self, bar, daily: pd.DataFrame, today) -> Optional[Dict]:
        """Detect stop hunt at 08:00-08:30."""
    ...
    def _calc_score(self, level: float, vwap: float, fibs: Dict, vol_hist: List) -> Tuple[float, Dict, str]:
        """Calculate institutional execution probability score."""
        bd = {}
        ztype = ""
        
        # 1. VWAP Distance (30 max)
        if vwap > 0:
        ...
        # 4. Volume (15 max) - simulated from history
        if len(vol_hist) >= 20:
            recent = np.mean(vol_hist[-10:])
        ...
        # 5. Delta (10 max) - simulated
        bd['delta'] = np.random.uniform(2, 7)
    ...
    def _open_position(self, zone: Dict, bar, balance: float) -> Dict:
        """Open position 7 pips before zone."""
    ...
    def _print_results(self, r: BacktestResult):
        ...
        if r.trades:
            print(f"\n📋 TRADE DETAYLARI (İlk {min(15, len(r.trades))}):")
            print(f"   {'Tarih':<14} {'Yön':<6} {'Skor':>5} {'Tip':<16} {'Pip':>7} {'K/Z':>10} {'Çıkış':<12}")
            print(f"   {'-'*76}")
            
            for t in r.trades[:15]:
                emoji = "✅" if t.profit > 0 else "❌"
                print(f"   {t.entry_time.strftime('%m/%d %H:%M'):<14} {t.direction:<6} {t.score:>5.0f} {t.zone_type[:16]:<16} {t.pips:>+7.1f} {t.profit:>+10.2f} {t.exit_reason[:12]:<12} {emoji}")
            
            if len(r.trades) > 15:
                print(f"   ... ve {len(r.trades) - 15} trade daha")
        ...
        target_reached = r.profit_pct >= 10
        dd_ok = r.max_drawdown_pct < 5
        total_dd_ok = r.max_drawdown_pct < 10
        
        print(f"   {'✅' if target_reached else '❌'} Kar Hedefi (%10):     {r.profit_pct:+.2f}%")
        print(f"   {'✅' if dd_ok else '❌'} Günlük DD (<5%):        {r.max_drawdown_pct:.2f}%")
        print(f"   {'✅' if total_dd_ok else '❌'} Toplam DD (<10%):       {r.max_drawdown_pct:.2f}%")
        
        if target_reached and dd_ok and total_dd_ok:
            print(f"\n   🎉 CHALLENGE GEÇİLDİ!")
        else:
            print(f"\n   ⚠️ Challenge başarısız")
        
        print(f"\n{'='*70}\n")


def main():
    """Run December 2025 backtest."""
    import argparse
    
    parser = argparse.ArgumentParser(description='IOFAE Backtester')
    parser.add_argument('--start', default='2025-12-01', help='Start date')
    ...
    try:
        ...
        if result.trades:
            ...
            filename = f"iofae_backtest_{args.start}_{args.end}.csv"
